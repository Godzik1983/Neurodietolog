...
PLATFORM_NAME = os.getenv("PLATFORM_NAME", "").strip().lower() or (
    "telegram" if os.getenv("TELEGRAM_TOKEN") else "max"
)
...
MANAGER_ESCALATION_TEXT = os.getenv(
    "MANAGER_ESCALATION_TEXT",
    "Передадим вопрос менеджеру для решения. Ожидайте обратной связи"
    if PLATFORM_NAME == "telegram"
    else "Спасибо за обратную связь! Передадим вопрос менеджеру для решения. Мы ценим ваше замечание и обязательно всё исправим."
)
...
def get_whisper_model():
    ...
    if whisper_model is None:
        whisper_model = WhisperModel(  # pyright: ignore[reportCallIssue]
            WHISPER_MODEL_SIZE,
...
def generate_dialog_reply(query: str, history: str, short_history: str, tone_of_voice: str, dialog_state: str,  system_prompt: str, model="gpt-4.1-mini", get_photo=False) -> str:
    ...
    if dialog_state == "positive_review_request":      
        ...
        state_instruction = (
            ...
            4. Затем попрощайся: Спасибо за обратную связь! Передадим вопрос менеджеру для решения. Мы ценим ваше замечание и обязательно всё исправим.{dialog_state == "stop"}"""
        )
...
def send_terminal_message(chat_id: int, bot: int, sender: int, text: str) -> None:
    result = send_text(chat_id, text)
    body = result.get("body", {}) if isinstance(result, dict) else {}
    save_message_to_db("outgoing", chat_id, bot, sender, body.get("mid"), body.get("seq"), text, raw_json=result)
        ...
        try:
            ...
            for upd in updates:
                ...
                if chat_info and chat_info.get("finish") == 1:
                    if meta["text"]:
                        send_terminal_message(chat_id, meta["bot"], meta["sender"], FINISHED_DIALOG_TEXT)
                    continue
